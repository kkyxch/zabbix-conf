...
HOST="r-2ze90edbc3756d04.redis.rds.aliyuncs.com"
PORT=6379
PASSWORD='TK2YvGwl'
...
if [[ $# == 1 ]];then
    ...
    case $2 in
        keys)
            result=`$REDISCLI -h $HOST -p $PORT -a $PASSWORD info | grep -w "$1" | grep -w "keys" | awk -F'=|,' '{print $2}'`
            echo $result
        ;;
        expires)
            result=`$REDISCLI -h $HOST -p $PORT -a $PASSWORD info | grep -w "$1" | grep -w "keys" | awk -F'=|,' '{print $4}'`
            echo $result
        ;;
        avg_ttl)
            result=`$REDISCLI -h $HOST -p $PORT -a $PASSWORD info | grep -w "$1" | grep -w "avg_ttl" | awk -F'=|,' '{print $6}'`
            echo $result
        ;;
        *)
